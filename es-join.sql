...
SELECT `students`.`name`, `students`.`surname`, IF(COUNT(`courses`.`name`)>1, 'più di un tentativo', 'primo tentativo') AS 'how_many_attemps', `courses`.`name` AS 'Course name', COUNT(`courses`.`name`) AS 'Student Attemps'
FROM `students`
JOIN `exam_student`
